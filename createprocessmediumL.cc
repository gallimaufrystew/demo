...
/// Creates a new process u16_Path with the integration level of the Explorer
/// process (MEDIUM IL)
/// If you need this function in a service you must replace FindWindow()
/// with another API to find Explorer process
/// The parent process of the new process will be svchost.exe if
/// this EXE was run "As Administrator"
/// returns Win32 API error or 0 if succeeded
DWORD CreateProcessMediumIL(WCHAR* u16_Path, WCHAR* u16_CmdLine)
{
	...
	/// Create the process normally (before Windows Vista or if
	/// current process runs with a medium IL)
	if (!b_UseToken)
	{
